...
==============================================================================
What this file does:
- Controls the entire game loop (title screen → level select → levels → results).
- Spawns all runtime objects (player, exes, friends, pads, UI text, backdrops).
- Keeps everything under one "RUNTIME" parent so I can Clear() and rebuild fast.

Why I went this route:
- I wanted to avoid juggling multiple Unity scenes while prototyping.
- Rebuilding levels in code makes iteration super fast (esp. with QuickRestart).

Key things I learned while building this:
- World-space UI needs its own Canvas and a higher sorting order to render on top.
- It’s easy to accidentally let the player slip off-screen, so I added barriers.
- Picking exit positions randomly can make "free wins", so I pick far corners.

Update summary (this pass):
- Added a YELLOW **EXIT GAME** tile to the Title/Splash screen.
- Added a **BACK** pad on Level Select to return to Title/Splash.
- Added QuitGame() helper that exits Play Mode (Editor) or quits the app (build).

Future TODOs:
- Swap PixelKit placeholders for real tiles/characters when art is ready.
- Convert these code-built levels to prefab/JSON-driven so designers can iterate.
- Add accessibility options (bigger FOV cones / color-blind outlines).
==============================================================================
*/

public class GameFlow : MonoBehaviour
{
    // Ensure we always have exactly one GameFlow
    ...
    // Track which level we’re in so QuickRestart() knows what to rebuild
    enum LevelId { None, Cafe, Campus, Street, Market, Metro, Party }
    LevelId currentLevel = LevelId.None;

    // Everything spawned during a screen/level lives under _root so Clear() wipes clean
    Transform _root;
    LevelRun _run;            // session stats (time, awkwardness) for grading
    PlayerController _player; // reference to the active player

    void Awake()
    {
        // Make sure a GameManager exists and listen for win/fail
    ...
    // Global quick restart (R key from PlayerController)
    ...
    public void BuildSplash()
    {
        ...
        // Playable avatar on the menu (keeps input consistent)
        ...
        // NEW: Exit Game (yellow) — bottom-right corner
        MakePad(new Vector3(6.5f, -4.5f, 0), 2, 2, 'e', "EXIT GAME", QuitGame);
    ...
    public void BuildLevelSelect()
    {
        ...
        // NEW: Back to Title/Splash
        MakePad(new Vector3(0f, -5.3f, 0), 2, 1, 'g', "BACK", BuildSplash);
    ...
    public void BuildLevel_Cafe()
    {
        ...
        BuildLevelCommon(
            "Goal: Reach the YELLOW EXIT. Avoid the Ex. E = hide / distract.",
            Backdrop_Cafe,
            () =>
            {
                BoundsWalls();
    ...
    public void BuildLevel_Campus()
    {
        ...
        BuildLevelCommon(
            "Goal: Cross the quad to the YELLOW EXIT. Q slows detection.",
            Backdrop_Campus,
            () =>
            {
                BoundsWalls();
    ...
    public void BuildLevel_Street()
    {
        ...
        BuildLevelCommon(
            "Goal: Slip past and reach the YELLOW EXIT in the alley.",
            Backdrop_Street,
            () =>
            {
                BoundsWalls();
    ...
    public void BuildLevel_Market()
    {
        ...
        BuildLevelCommon(
            "MARKET: Tight aisles. More patrols. Plan your dashes.",
            Backdrop_Market,
            () =>
            {
                BoundsWalls();
    ...
    public void BuildLevel_Metro()
    {
        ...
        BuildLevelCommon(
            "METRO: Platforms & trains. Very crowded patrols.",
            Backdrop_Metro,
            () =>
            {
                BoundsWalls();
    ...
    public void BuildLevel_Party()
    {
        ...
        BuildLevelCommon(
            "PARTY: Packed space. 6 patrols. Use friends & fake calls.",
            Backdrop_Party,
            () =>
            {
                BoundsWalls();
    ...
    public void BuildLevelCommon(string objective, System.Action backdrop, System.Action layout)
    {
        ...
        // Spawn player bottom-left-ish for consistency
        _player = SpawnPlayer(new Vector3(-8, -6, 0));

        // Let the level-specific layout add content (covers, exes, exit, friends)
        layout();

        // Track stats for end-of-level grade
        if (_run) Destroy(_run.gameObject);
        _run = new GameObject("LevelRun").AddComponent<LevelRun>();

        // Fade the objective after a short beat so it doesn't cover gameplay
        StartCoroutine(FadeObjectiveSoon(3f));
    }

    // Add invisible colliders at camera edges to keep the player in-frame
    void ScreenBarriers()
    {
        var cam = EnsureCamera();
    ...
    // Convenience patrol lane helper
    ...
    // coin-flip corner helper
    Vector3 RandomCorner(Vector3 a, Vector3 b) => (Random.value < 0.5f) ? a : b;

    // pick a far corner from a point (avoid free wins near spawn)
    ...
    // Old prototype stamina UI cleanup (prevents duplicates between screens)
    void PurgeStaminaUI()
    {
    ...
    // Tiny grade function based on time + awkwardness ratio
    ...
    // Freeze all gameplay systems before showing win/fail screens
    void FreezeGameplay()
    {
    ...
    // Remove the current runtime root (clears the screen)
    void Clear()
    {
    ...
    // Paranoid cleanup for stray objects left outside _root
    void NukeStrays()
    {
    ...
    // Generic pad spawner (green for select/continue, but we also use 'e' for yellow EXIT on splash)
    ...
    // Ex patrol with snappier FOV + basic animator
    GameObject Ex(Vector3 start, Vector3 wp1, Vector3 wp2)
    {
        ...
        fov.viewRadius = 7.5f;
        fov.viewAngle  = 110f;
        fov.visibilityFactor = 1.1f;
        fov.instantChaseDistance = 2.25f;
        fov.useVelocityAsForward = true;
    ...
    // Friendly lookout (visual helper)
    ...
    // Yellow EXIT tile (used in levels, not the splash quit)
    ...
    // Visual wall chunk with collider
    ...
    // Menu-only player (no DetectionMeter)
    ...
    // Make sure we have an orthographic Main Camera
    Camera EnsureCamera()
    {
        var cam = Camera.main;
        if (!cam)
    ...
    // Create an empty child under RUNTIME, optionally with a sprite and a position
    ...
    // Quick world-space text helper (no TMP dependency)
    ...
    // ===== Utility: Quit game (Editor-safe) =====
    void QuitGame()
    {
    #if UNITY_EDITOR
        UnityEditor.EditorApplication.isPlaying = false; // stop Play Mode in Editor
    #else
        Application.Quit(); // quit app in builds
    #endif
    }
}